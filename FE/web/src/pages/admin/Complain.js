...
const Request = () => {
  ...
  useEffect(() => {
    const fetchData = async (page) => {
      try {
        const response = await axios.get(`/products/report?pg_no=${page}&per_page=${postsPerPage}`);
        const data = response.data.data.rpts;
        
        // 데이터를 더 이상 가져올 필요가 없을 때
  ...
  }, [currentPage, postsPerPage, hasMore]);
  ...
  const handlePrevChunk = () => setCurrentPage((prevPage) => Math.max(prevPage - 5, 1));
  const handleNextChunk = () => setCurrentPage((prevPage) => prevPage + 1);
      ...
      <div className="content-container">
        ...
        <div className="content">
          ...
          <Pagination
            ...
            handlePrevChunk={handlePrevChunk}
            handleNextChunk={handleNextChunk}
            hasMore={hasMore} // 더 이상 가져올 데이터가 있는지 여부
          />
        </div>
      </div>
    </div>
